 ...
 *  Desc       : calculate orient distribution in MOFs simulation.
 */
...
public:
	...
	void calcVector(int nm, double v1[3], double v2[3])
	{
		for (int i = 0; i != 3; ++i)
		{
			v1[i] = periodicity(point1(nm, i) - point2(nm, i), Lbox[i]);
			v2[i] = periodicity(point1(nm, i) - point3(nm, i), Lbox[i]);
		}
	}
...
gmx_main(temp)
{
	...
	hd.pa = {
		...
		{ "-dim",FALSE, etINT, {&DIMN}, "dim of bin sets\ndim>2 means getting D in 3-dimension; 0(yz);1(xz); 2(xy);" },
		{ "-dbin", FALSE, etRVEC, {dbin}, "the distance of dbin (nm)"},
		{ "-low", FALSE, etRVEC, {low}, "the low position of region (nm)"},
		{ "-up", FALSE, etRVEC, {up}, "the up position of region (nm)"},
		{ "-c1",FALSE, etREAL, {&cylCenter1}, "the center of the circle1 (nm)" },
		{ "-c2",FALSE, etREAL, {&cylCenter2}, "the center of the circle2 (nm)" },
		{ "-dr",FALSE, etREAL, {&dr}, "the dr bin of R (nm)" },
		{ "-CR",FALSE, etREAL, {&CR}, "the up region of R (nm)" },
		{ "-Cr",FALSE, etREAL, {&Cr}, "the low region of R (nm)" },
		{ "-nA",FALSE, etINT, {&nAngle}, "number of bin sets of angle" },
	...
	double height = up[DIMN] - low[DIMN];
	double upPos = up[DIMN];
	double lowPos = low[DIMN];

	int nbin[3];
	for (int i = 0; i < 3; i++)
		nbin[i] = (up[i] - low[i]) / dbin[i];
	int nbin1 = (CR - Cr) / dr;
