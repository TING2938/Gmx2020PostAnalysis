...
gmx_main(temp)
{
	...
	hd.fnm = {
		{ efXVG, "-o", "rdf_time_in_pore", ffWRITE }
	};
	hd.init(); 
	hd.readFirstFrame(); 
	...
	fmt::print(file, "{:8.4f} ", 0.0);
	for (int i = 0; i != nbin; ++i)
	{
		fmt::print(file, "{:8.5f} ", Cr + i * dr + dr / 2);
	}
	fmt::print(file, "\n");
	...
	do
	{
		...
		fmt::print(file, "{:8.4f} ", hd.fr->time);
		for (int i = 0; i != nbin; ++i)
		{
			fmt::print(file, "{:8.5f} ", rdf[i] / volumn[i]);
		}
		fmt::print(file, "\n");
