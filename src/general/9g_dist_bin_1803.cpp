...
struct energy
{
    ...
    energy(size_t Num, double Vdw, double Cou, double R, size_t Frame = 0) :
        num(Num), vdw(Vdw), cou(Cou), r(R), frame(Frame)
    {
    }

    energy& add(size_t Num, double Vdw, double Cou, double R, size_t Frame = 0)
    {
        num += Num;
        vdw += Vdw;
        cou += Cou;
...
#define NFILE asize(fnm)
    ...
    std::vector<double> vdwi(nm1), coui(nm1), ri(nm1);
    std::map<size_t, energy> dist;
    std::map<size_t, energy> result;

    char outputFileName[60]; 

    sprintf(outputFileName, "Nr_%s_%s.dat", grpname[0], grpname[1]);
    ...
    fprintf(file, "\n# tN1= %10s , tN2= %10s", grpname[0], grpname[1]);
    fprintf(file, "\n# nm1= %10d , nm2= %10d", nm1, nm2);
    auto pos = ftell(file);
    fprintf(file, "%50s\n#%4s%5s%15s%15s%15s\n", " ", "atom", "Num", "vdw", "cou", "r");
    ...
    do {
        ...
        std::fill_n(vdwi.begin(), nm1, 0);
        std::fill_n(coui.begin(), nm1, 0);
        std::fill_n(ri.begin(), nm1, 0);
        fprintf(file, "# time = %f\n", fr.time);
        ...
        for (i = 0; i < nm1; i++) {
            ...
            if (lowPos < dL[DIMN] && dL[DIMN] < upPos) {
                ...
                if (Number[i] != 0) {
                    ...
                    vdwi[i] = Vv1[i] / Number[i];
                    coui[i] = Cc1[i] / Number[i];
                    ri[i] /= Number[i];
                    //Vv2[meZ] += Vv1[i] / 1;
                    //Cc2[meZ] += Cc1[i] / 1;
                }
            ...
            dist[Number[i]].add(1, vdwi[i], coui[i], ri[i]);
            fprintf(file, "%5d%5d%15.4e%15.4e%15.4e\n", i, Number[i], vdwi[i], coui[i], ri[i]);
        }

        for (auto&& it : dist) {
            auto &sec = it.second;
            result[it.first].add(sec.num, sec.vdw / sec.num, sec.cou / sec.num, sec.r / sec.num, 1);
        }
    ...
    for (auto&& it : result) 
        fprintf(outputfileR, "%15zd%15.5f%15.5f%15.5f%15.5f\n", 
                it.first, it.second.num / double(index), it.second.vdw / it.second.frame, 
                it.second.cou / it.second.frame, it.second.r / it.second.frame);
