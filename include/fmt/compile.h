...
namespace internal
{
	...
	public:
		...
		enum class kind
		{
			argument_id, named_argument_id, text, specification
		};
	...
	template <typename Char> struct part_counter
	{
		...
		FMT_CONSTEXPR void on_arg_id(unsigned)
		{
			++num_parts;
		}
	...
	public:
		template <unsigned N> struct format_parts_array
		{
			...
			FMT_CONSTEXPR value_type& operator[](unsigned ind)
			{
				return arr[ind];
			}

			FMT_CONSTEXPR const value_type* begin() const
			{
				return arr;
			}
			FMT_CONSTEXPR const value_type* end() const
			{
				return begin() + N;
			}
	...
	public:
		...
		FMT_CONSTEXPR void push_back(format_part part)
		{
			parts_[counter_++] = part;
		}

		FMT_CONSTEXPR format_part& back()
		{
			return parts_[counter_ - 1];
		}
